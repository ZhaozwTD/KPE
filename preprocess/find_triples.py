...
def obtain_vocab(model_name):
    ...
    with open(config["paths"]["plm_vocab"], 'w', encoding='utf-8') as f:
...
def filter_triples(triples, threshold=3.5, max_triples=50):
    N = len(triples)
    if N == 0:
        return triples
    else:
        rel_count = Counter([t[1] for t in triples])
        all_weights = [(t, t[3] * N / rel_count[t[1]]) for t in triples]
        all_weights = sorted(all_weights, key=lambda x: -x[1])
        res = [t[0] for t in all_weights if t[1] > threshold]
...
def find_triples(sent, topic_prompt, is_self_extract=True):
    ...
    return filter_triples(triples, threshold=float(config["parameters"]["threshold"]),
                          max_triples=int(config["parameters"]["max_triples"]))
...
def process(input_path, output_path, type, dataset_name):
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        ...
        for data in tqdm(data_list, desc='extract triples for {} dataset'.format(type)):
            query = data['query']
            if type == 'test':
                answer = []
            else:
            ...
            examples.append({'query': query, 'cands': cands, 'answer': answer, 'triples': triples})
...
for data_type in TYPES:
    ...
    out_path = os.path.join(DATA_PATH, '.'.join([data_type, dataset_name, 'triples', 'json']))
